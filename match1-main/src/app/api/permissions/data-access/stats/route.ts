...
export async function GET(request: Request) {
  try {
    ...
    // 获取资源访问统计（基于操作类型分类）
    const resourceAccessMap = new Map<string, { total: number; allowed: number; denied: number; users: Set<string> }>();
    ...
    allAuditLogs.forEach(log => {
      // 从action中提取资源类型 (e.g., "competition:create" -> "competition")
      const resource = log.action.includes(':') ? log.action.split(':')[0] : 'system';
      const isAllowed = !log.action.includes('FAILED') && !log.action.includes('DENIED');

      if (!resourceAccessMap.has(resource)) {
        resourceAccessMap.set(resource, { total: 0, allowed: 0, denied: 0, users: new Set() });
      }
      const stat = resourceAccessMap.get(resource)!;
      stat.total++;
      if (isAllowed) stat.allowed++;
      else stat.denied++;
      stat.users.add(log.userId);
    });

    const resourceStats = Array.from(resourceAccessMap.entries()).map(([resource, stat]) => ({
      resource,
      totalAccess: stat.total,
      allowedAccess: stat.allowed,
      deniedAccess: stat.denied,
